...
After producing the audit report, extract observable behaviours from the **input spec** (not from the gaps — gaps are problems to fix, not things to verify) and write a checklist file.

**What to include:** every behaviour the input describes that is directly testable via HTTP request. One item per observable outcome. Skip architectural intent, "how it works" descriptions, and anything that can't be confirmed from the outside.

**Do NOT include:** gap items, contradictions, or missing cases flagged in the report — those must be resolved before they can be verified.

**Write to:** `<project-root>/.checklist/audit-<YYYYMMDD-HHMMSS>.md`

If not in a git repo, write to `./.checklist/audit-<YYYYMMDD-HHMMSS>.md`.

**File format:**
```markdown
# Acceptance Checklist
<!-- Generated by /audit on <YYYY-MM-DD HH:MM:SS> -->
<!-- Source: <one-line description of what was audited> -->
<!-- Note: N gap(s) found — resolve them before running /verify -->
...
`Checklist skipped — no HTTP-verifiable behaviours found in input.`
