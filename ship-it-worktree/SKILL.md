...
All orchestrator state lives in `./ship-it-worktree-<PRD_SLUG>.json`. The Markdown dashboard is derived from it. Re-read before each write to handle near-simultaneous agent completions.
...
**Resume check:** If `./ship-it-worktree-<PRD_SLUG>.json` exists and `--prd` is omitted, offer to resume (§resume).
...
Write `./ship-it-worktree-<PRD_SLUG>.json` with all task data (see §session-state schema above).

### 3b. Write Markdown dashboard

Write `./ship-it-worktree-<PRD_SLUG>.md`:
...
When `./ship-it-worktree-<PRD_SLUG>.json` exists:

```
Found existing ship-it-worktree session: <prdTitle>
  Done:        N
  In-progress: N
  Pending:     N
  Failed:      N
...
```bash
gh issue create \
  --title "[ship-it-worktree FAILED] <displayTitle>" \
